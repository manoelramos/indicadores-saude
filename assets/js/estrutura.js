const = {

    nome: Informação para o SUS: políticas e sistemas;



Módulo 1: Políticas Sociais e Informação
Aula 1 – Informação para as políticas sociais
	Apresentação do curso
	Objetivos de aprendizagem
	Introdução
	Informações para políticas sociais
	Política Nacional de Informação e Informática em Saúde (PNIIS)
	Considerações finais

Aula 2 – Informação científica e tecnológica em saúde
	Conteúdo da aula
	Objetivos de aprendizagem
	Apresentação
	Informação: principais antecedentes e conceitos
	Conceito de informação
	Funções da Informação
	Principais correntes teóricas da informação
	Teoria matemática
	Teoria sistêmica
	Teoria crítica da informação
	Fontes de informação
	Tipologias e características das fontes
	Fluxo da informação (ou ciclo)

Aula 3 – Políticas da Informação
	Conteúdo da aula
	Objetivos da Aprendizagem
	Apresnetação
	Introdução à Lei Geral de Proteção de dDdos (LGPD) e marcos regulatórios da informação para o controle social
	Antecedentes da Lei Geral de Proteção de Dados (LGDP)
	Conceitos básicos
	A Lei Geral de Proteção de Dados (LGPD)
	Princípios da LGPD
	Requisitos para tratamento de dados pessoais
	Requisitos para tratamento de dados sensíveis
	Direito dos titulares de dados
	Obrigações do Controlador e Operador de Dados
	Desafios da proteção de dados na Saúde Digital
	Boas práticas e recomendações
	Entender a informação como direito do cidadão
	Intersetorialidade como espaço de compartilhamento de saber e poder


Aula 4 – A informação no Sistema Único de Saúde
	Políticas para informação
	Conteúdo da aula
	Objetivos de aprendizagem
	Apresentação
	Histórico e marcos legais da informação em saúde no SUS
	A organização, responsabilidade e fluxo dos dados em saúde
	A organização do Sistema Nacional de Informação em Saúde (SNIS)
	Transformação digital na saúde pública
	A organização, responsabilidade e fluxo de dados em saúde


Módulo 2: Sistemas de informação de saúde e Fonte de Dados de interesse à saúde:
Aula 1 – Políticas para a Informação
	Conteúdo da aula
	Objetivos de aprendizagem
	Apresentação
	Sistemas de Informação em Saúde I
	Sistema de Informação sobre Mortalidade (SIM)
	Coleta e processamento dos dados sobre mortalidade
	Sistema de Informações sobre Nascidos Vivos (SINASC)
	Cadastro Nacional de Estabelecimento de Saúde (CNES)


Aula 2 - Sistemas de Informação II (SIH, SINAN, SISAB, SIVEP)
	Conteúdo da aula
	Objetivos de aprendizagem
	Introdução
	Sistema de Informação de Internações Hospitalares (SIH/SUS)
	Sistema de Gerenciamento da Tabela de Procedimentos, Medicamentos, Órteses/Próteses e Materiais Especiais do SUS (SIGTAP)
	Sistema de Informações Ambulatoriais (SIA/SUS)
	Sistema de Informações de Agravos de Notificação (SINAN)
	Sistema de Informação da Vigilância Epidemiológica da Gripe (SIVEP-Gripe)
	Sistema de Informação em Saúde para a Atenção Básica (SISAB) e e-SUS Atenção Primária à Saúde (e-SUS APS)


Aula 3 - Sistemas nacionais de Informação de interesse a saúde
	Conteúdo da aula
	Objetivos de aprendizagem:
	Introdução
	Importância dos Dados para Políticas de Saúde
	Sistemas de Informações de interesse à saúde
	Dados Demográficos
	Dados Socioeconômicos
	Dados sobre Fatores de Risco, Exposição e Populações Vulneráveis
	Dados de recursos e cobertura
	Considerações Finais

Aula 4 - Indicadores de Saúde
	Conteúdo da aula
	Objetivos de aprendizagem:
	Conceitos e Definições Básicas
	O que é um indicador?
	Construção de indicadores
	Índices (Indicadores Compostos ou Sintéticos)
	Atributos desejáveis de um indicador
	Fichas de qualificação de indicadores
	Sistemas de indicadores

Módulo 3: Integração de dados, novas fontes e uso
AULA 1 - Interoperabilidade de padrões em Informática em Saúde
Conteúdo da aula
	Objetivos de Aprendizagem
	Definição de interoperabilidade
	A interoperabilidade na prática
	Implementação do e-SUS
	Sistema de integração de informações em tempo real
	Rede nacional de dados em saúde (RNDS)
	Políticas publicas e interoperabilidade
	Desafios e perspectivas futuras
	Tendências globais
	Perspectivas futuras


AULA 2 – Rede Nacional de Dados em Saúde
	Conteúdo da aula
	Objetivos de aprendizagem
	Introdução
	Objetivo da RNDS
	Contexto da Pandemia – Uso da RNDS durante a Pandemia da Covid-19 e o passaporte de vacinação
	Problemas de interoperabilidade
	Estrutura da RNDS
	Componentes e funcionalidades das RNDS
	Governança e Segurança
	Recomendação da OCDE e Governança Internacional de dados de saúde
	Evolução dos Comitês de Governança das TIC no Ministério da Saúde
	Melhoria do Atendimento e Ferramentas de Análise
	Considerações fiais – perspectivas futuras da RNDS na saúde pública.


AULA 3 – Fontes de dados digitais: barramentos de interoperabilidade
	Objetivos de Aprendizagem
	Introdução
	Importância e contextualização
	Padrões e protocolos essenciais
	Desafios na implementação
	Conecte SUS
	Aplicações e estudos de casos internacionais
	Prontuários eletrônicos do Paciente (PEP)
	Normas e Diretrizes
	Serviços de Saúde Digital
	Desafios e soluções
	Conclusão

AULA 4 – Interoperabilidade para as políticas sociais: exemplos práticos
	Objetivos de aprendizagem
	Introdução
	Introdução à interoperabilidade em saúde
	Exemplos práticos
	Plataformas e sistemas
	Infraestrutura de Nacional de Dados (IND)
	Eixos da Infraestrutura Nacional de Dados
	Objetivos da Infraestrutura Nacional de Dados
	Rede Nacional de Dados em Saúde (RNDS)
	Interoperabilidade em Saúde: integrando atenção e tecnologia no cuidado ao paciente.
	Interoperabilidade na Atenção Primária, Secundária e Terciária.
	Interoperabilidade e o paciente: desafios e oportunidades.
	Interoperabilidade e o futura da saúde no Brasil
	Conclusão

}
